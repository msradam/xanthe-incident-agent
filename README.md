...
[Xanthe](https://github.com/msradam/xanthe); the agent connects as an MCP client and can
only take the moves the machine allows. The division of labor:
...
Claude is handed the MCP tools (`step`, `state`, `reset`, `fork_at`, `fork_from_past`) and
drives the incident itself: reading state, choosing moves, recovering from refusals, and
writing the postmortem. It is held to the runbook the whole way.
